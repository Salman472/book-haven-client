...
const Comment = () => {
  ...
  useEffect(() => {
    axios(
      `https://book-haven-server-bay.vercel.app/all-books/new-comment/${_id}`
    )
      .then((data) => {
  ...
  const hendleComment = (e) => {
    ...
    if (comment === "") {
      return toast.success(
        "Please write something before submitting your comment!"
      );
    }
    // console.log(comment);
    const commentInfo = {
    ...
    axios
      .post("https://book-haven-server-bay.vercel.app/new-comment", commentInfo)
      .then((data) => {
        console.log(data.data);
